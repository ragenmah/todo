...
class DatabaseHelper {
  ...
  String todoTable = 'todo_table_new';
  String colId = 'id';
  ...
  Future<Database> initializeDatabase() async {
    ...
    String path = directory.path + 'todos_new.db';
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute(
        ...
        '$colDescription TEXT, $colDate TEXT,$colComplete INTEGER)');
  }
...
//		var result = await db.rawQuery('SELECT * FROM $todoTable order by $colTitle ASC');
  ...
  Future<List<Map<String, dynamic>>> getTodoTodayMapList() async {
    Database db = await this.database;
    String _date = DateFormat.yMMMd().format(DateTime.now());

    // var result = await db.rawQuery(
    //     'SELECT * FROM $todoTable where date=$date order by $colTitle ASC');
    var result = await db.query(todoTable,
        where: "date=?", whereArgs: [_date], orderBy: '$colTitle ASC');
    return result;
  }

  Future<List<Map<String, dynamic>>> getTodoTomorrowMapList() async {
    Database db = await this.database;
    String _date =
        DateFormat.yMMMd().format(DateTime.now().add(new Duration(days: 1)));
  ...
  Future<List<Todo>> getTodoList(String tabDetail) async {
    // var todoMapList = await getTodoMapList(); // Get 'Map List' from database

    var todoMapList; // Get 'Map List' from database

    switch (tabDetail) {
      case "all":
        todoMapList = await getTodoMapList();
        break;
      case "today":
        todoMapList = await getTodoTodayMapList();
        break;
      case "tomorrow":
        todoMapList = await getTodoTomorrowMapList();
        break;
      default:
    }
